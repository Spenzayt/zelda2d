...
void Camera::update(const sf::Vector2f& playerPosition, float deltaTime) {
    ...
    if (inRoom) {
        ...
        if (x > currentRoom.left + currentRoom.width - view.getSize().x / 2) {
            x = currentRoom.left + currentRoom.width - view.getSize().x / 2;
        }
        if (y > currentRoom.top + currentRoom.height - view.getSize().y / 2) {
            y = currentRoom.top + currentRoom.height - view.getSize().y / 2;
        }
